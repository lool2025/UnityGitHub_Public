...
namespace MFarm.AStar
{
    public class AStar : Singleton<AStar>
    {
        ...
        private List<Node> openNodeList;//当前被选中Node附近的8个点
        ...
        {
            if (currentNode.gridPosition == null)
            {
                Debug.Log("[nullgridPosition]");
                }
            ...
            for(int x = -1;x<=1;x++)
            {
                for(int y = -1;y<=1;y++)
                {
                    ...
                    validNeighbourNode=GetValidNeighourNode(currentNodePos.x +x, currentNodePos.y +y);
        ...
        /// <param name="y"></param>
        /// <returns></returns>
        private Node GetValidNeighourNode(int x,int y)
        {
